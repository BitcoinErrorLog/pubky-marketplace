#!/bin/sh
# Composed-environment Lock Server entrypoint.
#
# Identical to the upstream docker/locks-server-compose-entrypoint.sh at
# locks@ba49a777, with two additions:
#   1. the generated compose config gains a [paykit] section so the Lock
#      Server creates real payment lifecycles against the Paykit Server;
#   2. the Paykit Server config is generated into the shared volume once the
#      Lock Server identity exists, following the generated-config contract in
#      paykit-server docs/local-locks-demo.md (trusted_public_key must be the
#      exact canonical credentials.lock_server_public_key value).
set -eu
...
paykit_shared_dir="${PAYKIT_SHARED_DIR:-/paykit-shared}"
paykit_server_url="${LOCKS_PAYKIT_SERVER_URL:-http://127.0.0.1:3001}"
...
if [ -d "$paykit_shared_dir" ]; then
  echo "[locks-compose] writing generated Paykit Server config to $paykit_shared_dir"
  printf '%s\n' "$lock_server_public_key" > "$paykit_shared_dir/lock_server_public_key"
  cat > "$paykit_shared_dir/paykit-server.toml.tmp" <<EOF
[http]
listen_addr = "0.0.0.0:3001"

[locks]
trusted_public_key = "$lock_server_public_key"
...
poll_interval = "500ms"
EOF
  chmod 644 "$paykit_shared_dir/paykit-server.toml.tmp"
  mv "$paykit_shared_dir/paykit-server.toml.tmp" "$paykit_shared_dir/paykit-server.toml"
else
  echo "[locks-compose] paykit shared dir $paykit_shared_dir missing; skipping Paykit config generation" >&2
fi
