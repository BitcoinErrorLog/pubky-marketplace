...
- **Live proofs, not mocks**: a re-runnable real purchase over the composed regtest stack (`npm run test:marketplace:locks`) — additionally proven on the DEPLOYED Railway rails against the real staging homeserver, live client-vs-service auth/command verification (`npm run test:marketplace:service`), 16/16 browser e2e for encrypted messaging plus live two-party testnet/staging proofs, a live attested-review-to-Nexus proof (`npm run test:marketplace:reviews`), a live FCFS drop two-buyer race on the deployed staging stack (`npm run test:marketplace:drops`, 2026-08-23), and the service's own integration suite with concurrency proofs

## Honest limits

- **No independent security review** (explicitly waived for now; still required before real funds; regtest coins only today)
- **Guarded-content durability across days**: the Bitkit wallet leg is fully proven (2026-08-22 — real in-app claim, Payment Request, swipe-to-pay, on-chain confirmation), but that proof surfaced a day-old guarded upload answering 404 until re-uploaded; a targeted durability probe is still owed before cross-day digital delivery is relied on
