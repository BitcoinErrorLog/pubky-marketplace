...
| Wave 7 rollout (2026-09-08) | Service `462ed54` deployed to PRODUCTION (deployment `396128db`), `PICKUP_DETAILS_ENCRYPTION_KEY` set there; `/health` reports `pickup_available:true` (sandbox off). Shop client `383b6d3e` (7.2a data layer `691b810f`+`5a6454f2`, 7.2b UI `2aea0170`..`383b6d3e`) deployed to staging `shop.pubky.app` and to the production alias; every slice passed Opus + Kimi gates (7.2a: Opus FIX-FIRST → SHIP, Kimi SHIP; 7.2b: Opus FIX-FIRST → SHIP after two fix rounds, Kimi SHIP ×2). Live check: staging PASS on all rows; production catalog cards render but a staging-owned listing opens "Listing unavailable" — see the Nexus note below. Sellers whose pre-Wave-7 listings were published as pickup (12 on staging) must set a meeting point on the edit page before pickup orders can complete. | `mp-ux`, `mp-prod-deploy`, `marketplace-service` |
| Production Nexus residue (OPEN, owner decision) | Production `nexusd` (`95a0`) serves the same 26 listings as staging: stale staging-derived rows in production Redis/Neo4j from the earlier default-config crash-loop; current `NEXUS_HOMESERVER` is the production homeserver and both stores are project-private. Correction per `docs/production-cutover.md`: wipe production Redis + Neo4j volumes, redeploy `nexusd`, replay from cursor zero (~1 day). Destructive — not applied. Until then the production alias shows staging listings and their detail pages read "Listing unavailable". | `pubky-nexus` |
