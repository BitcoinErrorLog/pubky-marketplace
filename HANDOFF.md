...
| Same-site bridge rehearsal | Vercel project `pubky-app-bridge-rehearsal`, branch `feat/session-bridge` in the fork checkout `/Volumes/vibedrive/vibes-dev/pubky-app` (current HEAD via `git -C /Volumes/vibedrive/vibes-dev/pubky-app log -1 --oneline feat/session-bridge`), domain `https://bridge.pubky.app`; Shop rehearsal Vercel project `shop-bridge-rehearsal`, worktree `mp-bridge-rehearsal`, branch `marketplace/bridge-rehearsal`, domain `https://shop-rehearsal.pubky.app` | Not a transaction-service change; rehearsal tests session bridge handoff between pubky.app and Shop | Uses Shop rehearsal env `NEXT_PUBLIC_VIBE_SESSION_BRIDGE_ORIGIN=https://bridge.pubky.app` and `PUBKY_RUNTIME_DEFAULT_URL`; pubky.app rehearsal env allows `https://shop-rehearsal.pubky.app` | Both domains verified 2026-09-06 13:16 UTC+1 and serve HTTP 200; any new `*.pubky.app` host needs a `_vercel.pubky.app` TXT record `vc-domain-verify=<host>,<token>` because the `pubky.app` apex is owned by another Vercel team |

### Tree ↔ Stack

| Tree | Stack |
| --- | --- |
| `mp-ux` | Vercel `pubky-marketplace-staging`; alias `pubky-marketplace-staging.vercel.app` |
| `mp-prod-deploy` | Vercel `pubky-marketplace-production`; public host `shop.pubky.app` plus alias `pubky-marketplace-production.vercel.app` |
| `mp-oneauth` | Not a deploy tree. Branch `marketplace/one-approval` ships `docs/ecommerce/single-approval.md` (feature `c213ae9f`, tests `454828c5`, fixer round `454828c5..3e737732`, cleanup batch `e482e603..8881f90f`, N-1 follow-up `ac593082`) |
| `mp-bridge-rehearsal` | Vercel `shop-bridge-rehearsal` |
| `pubky-app` fork branch `feat/session-bridge` | Vercel `pubky-app-bridge-rehearsal` |
| `marketplace-service`, `pubky-payment-rails`, `pubky-fiat-verifier` | Railway staging project `pubky-marketplace-staging` (`c991d768-4a3c-42ea-b5ed-eaa22d4916ed`); Railway environment is named `production` |
...
Vercel CLI: use `vercel` or `npx vercel`. Team scope `synonymdev` is the deployment account, orgId `team_y2cqjCWZ9vTnCPWQkUAgfijD`. Each worktree's Vercel project linkage is discovered from its `.vercel/project.json`.
...
Source pointers in `mp-ux`: `PUBKY_RUNTIME_*` names and defaults live in `src/libs/runtime-config/runtime-config.schema.ts`; `NEXT_PUBLIC_*` build-time names live in `src/libs/env/env.ts`; Sentry runtime and source-map variable names are documented in `docs/sentry.md`.
...
| --- | --- | --- |
| Shop staging | From `mp-ux` on `marketplace/pr25-ux`: `vercel deploy --prod --scope synonymdev` or `npx vercel deploy --prod --scope synonymdev` for project `pubky-marketplace-staging` | Vercel `vercel rollback <deployment-url|id>` or `vercel promote <deployment-url|id>` for the target deployment |
| Shop production | From `mp-prod-deploy` on `marketplace/prod-deploy` or the intended integration branch: `vercel deploy --prod --scope synonymdev` or `npx vercel deploy --prod --scope synonymdev` for project `pubky-marketplace-production` | Vercel `vercel rollback <deployment-url|id>` or `vercel promote <deployment-url|id>`; moving `shop.pubky.app` is parent/owner-only |
| Marketplace service staging/production | From `marketplace-service`: `railway up -p <project-id> -e production -s marketplace-service` after selecting the correct project (`pubky-marketplace-staging` or `pubky-marketplace-production`) | Railway dashboard rollback to a known-good deployment; `railway down` is not a rollback |
...
`mp-ux/docs/ecommerce/runbook-production.md` (committed at fixed doc artifact `5a1312f7` on `marketplace/pr25-ux`). The runbook also records Railway guidance: use dashboard rollback for an older known-good deployment; `railway down` removes the latest deployment and is not a rollback. The client kill switch is
...
| `node pubky-payment-rails/verify/driver.mjs checkout <bundle> paypal` | Hosted PayPal sandbox purchase through deployed rails. The driver has no `--help`; document this checkout invocation only. |
| `node mp-probe/scripts/probe-production-writes.mjs` | Production path-family PUT/GET/DELETE probe with Ring sign-in and `/priv` durability seed. Lives on the unmerged branch `marketplace/prod-probe` (worktree `mp-probe`, off `pr25-ux`); merges into `pr25-ux` once the owner has run it. |
...
| --- | --- | --- |
| Toast copy rule | **Shipped 2026-09-09.** Marketplace toasts and rendered failure text map machine-readable reasons to static, client-owned strings in `src/libs/commerce/failure-messages.ts`; server-supplied error messages never reach the toast or `errorMessage`, while typed client-side validation messages remain verbatim. Commits `ea62f5e9`, `f0d82dba`, `7dcdd63d` (production merge `4607a692c`, deploy `pubky-marketplace-production-ccat1hufk`, alias `shop.pubky.app`; staging `pubky-marketplace-staging-k7uzbc06f`), then cleanup `148db15ed` and `91e041c9c` in the following `pr25-ux` deploy. | `mp-toasts` |
| DM `sent_at` | **Shipped 2026-09-09.** `pubky_app.dm.v0` messages emit `sent_at` as a Unix-millisecond integer; inbound parsing accepts integer or legacy ISO strings under a bounded-range guard, and legacy ISO rows normalize on read. Commits `ce566979` and `c694b192`; Kimi audit SHIP with four non-gating findings fixed in `c694b192`. Production deploy `pubky-marketplace-production-371i2l41j`, staging deploy `gm8z9ndvp`; superseded by the `4607a692c` deploy above. | `mp-toasts` |
...
### Toast and messaging known gaps

- The buyer copy at `src/core/application/commerce/commerce.ts:644` still renders a seller-declared origin string; this remains backlog and was not part of the toast copy shipment.
- `error.factories` and Sentry still carry the raw error message in error context; the message is scrubbed by a denylist, not removed. This remains backlog.
...
### 2026-09-09 Evidence Rows

| Shop root redirect | Shipped on `marketplace/pr25-ux` at `e6ffcf6e`; `/` returns `307` to `/marketplace` (`200`), with production deploy `pubky-marketplace-production-fg6z17e36` / alias `shop.pubky.app` and staging `pubky-marketplace-staging-1iabskwrx`. | `mp-ux` |
| Shop Bitcoin auto-hide | Shipped at `e6ffcf6e` as hop 3: Bitcoin requires both `bitcoin_available` and `bitcoin_offer_available`; hop 1 (`/health/ready` publication) is in progress and hop 2 (marketplace-service consumption/cache plus no payment-config `503` during Paykit outages) is implemented, under review, and undeployed. The absent field defaults to `true`. | `mp-ux` |

### 2026-09-06 Evidence Rows

| Evidence | Status | Where |
| --- | --- | --- |
| Buyer UX fixes | Shipped on `marketplace/pr25-ux`: Orders entry for signed-in users, approval card removed from passive listing view, approval requested only after Add to cart / Place a bid / Make offer requires it, and copy standardized to `Approve purchases in Pubky Ring` / `Approve in Pubky Ring`. Check the current branch head with `git log -1 --oneline` from `mp-ux`. | `mp-ux` |
...
- Production attestor cross-check: `szhtpayftdz3mpkoyyk3zesuad11ufuudqqrc73s35w1tfju7gxy` is pinned in `mp-ux/src/config/commerce.ts` alongside staging `ws343aqzmcahagojhmhkbri8odqz9iqg61woxbkh9fd3bxhqomdy`.
